---
title: Nami
slug: nami
description: "A wave-themed wallpaper pack with 10 variants."
tag: "Released on Gumroad"
tagShort: "released"
date: "2021-01-10"
---

<h2>Timeline</h2>
<section id="timeline">
  <section className="released">
    <h3>Released on Gumroad</h3>
    <p>
      For those who want to help me pay for personal projects like Mello and this website (or just want some neat wallpapers), this pack launched on Gumroad. A link to the Gumroad page in the &quot;Useful links&quot; section below.
    </p>
    <h4>January 10th, 2021</h4>
  </section>
  <section className="progress">
    <h3>Started designing</h3>
    <p>
      This is my first wallpaper pack, and I&apos;m already having a smashing good time with doing this up for you guys.
    </p>
    <h4 className="minor">December 15th, 2020 to December 22nd, 2020</h4>
  </section>
  <section className="progress">
    <h3>Conceptialized</h3>
    <h4 className="minor">December 14th, 2020</h4>
  </section>
</section>

### Useful links
- [Gumroad](http://gum.co/lzjVt)

---

I have this addiction, so to speak, of changing wallpapers. I love seeing something new everytime I turn on my tech. Wanting to give myself some new variety, I did up my first wallpaper pack: Nami.

![Watermarked cover of Nami](/assets/projects/nami/cover0.png)

Nami (which means &quot;Waves&quot;) is a set of 10 different coloured and organic waves. Over almost a week now, I've been working on crafting these to be have great colours and a unique look.

![Watermarked cover of Nami](/assets/projects/nami/cover1.png)

I'm not one for asking for lots of money, even on something as simple as wallpapers. So, this pack is PWYO (pay-what-you-want) with a minimum of 1$ USD. This lets me still get a little something for my work, while still leaving it open to people who might buy the pack to spend whatever they feel it's worth.
